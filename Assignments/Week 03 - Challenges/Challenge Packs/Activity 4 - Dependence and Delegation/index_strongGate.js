...
function validateContract(parsed) {
  ...
  // Check that 'assumptions' exists and is an array
  ...
  // Check that 'what_i_would_need_to_be_sure' exists and is an array
  if (!Array.isArray(parsed.what_i_would_need_to_be_sure)) {
    errors.push("'what_i_would_need_to_be_sure' must be an array");
  }
...
async function main() {
  ...
  // Gate 2: Does it have the required fields with correct types?
  const validation = validateContract(parsed.value);
  if (!validation.ok) {
    console.error("CONTRACT FAILED: JSON is missing required fields or has wrong types.");
