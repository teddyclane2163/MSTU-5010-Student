...
function usage() {
  return [
    'Usage: node index.js --prompt "..." [--format text|json] [--temperature 0.7] [--debug] [--dry-run] [--log logs/run.json]',
    "",
    "Environment:",
...
function systemMessage() {
  return [
    "You are a perspective-taking assistant.",
    "Produce two perspectives with equal care.",
    "Do not assume one is 'correct'.",
    "Be concise.",
  ].join(" ");
}

function perspectiveContract() {
  return [
    "Output STRICT JSON (no markdown, no extra keys):",
    '{ "student_view": string, "instructor_view": string, "tradeoffs": string[] }',
  ].join(" ");
}

function buildRequest({ model, prompt, temperature, format }) {
  const userContent = format === "json" ? `${prompt}\n\n${perspectiveContract()}` : prompt;
  return {
    model,
    messages: [
