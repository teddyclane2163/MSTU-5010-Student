...
// One clear, inspectable interface for students:
// Input: a sound filename (e.g., "happy.mp3")
// Output: background audio should loop until a new filename arrives.
function createMusicController({ soundsDir, debug = false }) {
  ...
  async function pickBackend() {
    ...
    backend = (await createFfplayBackend({ log })) || (await createMacAfplayBackend({ log })) || (await createWindowsWmpBackend({ log })) || createNoAudioBackend({ log });
    return backend;
  }

  async function setSound(soundFilename) {
    const b = await pickBackend();
    if (soundFilename === current) return;
    current = soundFilename;
    const p = resolveSoundPath(soundFilename);
    // Important: never let multiple tracks stack. Always stop (with best-effort fade) before starting.
    await b.switchTo(p);
  }
...
function createNoAudioBackend({ log }) {
  ...
  return {
    ...
    async switchTo(filePath) {
      await this.playLoop(filePath);
    },
    async playLoop(filePath) {
      if (!warned) {
        warned = true;
...
async function killProcessTree(proc, { log }) {
  ...
  if (process.platform === "win32") {
    // Kill the whole tree to prevent layered audio from orphaned subprocesses.
    try {
  ...
  // On Unix, kill the process itself. (We intentionally avoid detached process groups so
  // child audio processes die when the parent Node process dies.)
  try {
  ...
  // Best-effort hard kill if still alive after a short delay.
...
async function createMacAfplayBackend({ log }) {
  ...
  return {
    ...
    async switchTo(filePath) {
      ...
      // afplay has no built-in loop. We implement looping in Node so we can stop cleanly
      // (and avoid a shell loop that can continue after the terminal is closed).
      shouldLoop = true;
      const start = () => {
...
async function createWindowsWmpBackend({ log }) {
  ...
  // Use a long-lived PowerShell process so we can fade + switch tracks without restarting the player.
